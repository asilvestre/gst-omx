  ...
  if ((buf->omx_buf->nFlags & OMX_BUFFERFLAG_CODECCONFIG)
    ...
    if (frame) {
      ...
      if (buf->omx_buf->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) {
        GST_DEBUG_OBJECT(self, "Passing buffer as frame complete - nFlags %04X, size %d",
            buf->omx_buf->nFlags,
            gst_buffer_get_size(frame->output_buffer));
        flow_ret =
            gst_video_encoder_finish_frame (GST_VIDEO_ENCODER (self), frame);
      } else {
        GST_DEBUG_OBJECT(self, "NOT passing buffer as it is a fragment - nFlags %04X, now size %d",
            buf->omx_buf->nFlags,
            gst_buffer_get_size(frame->output_buffer));
      }
    } else {
